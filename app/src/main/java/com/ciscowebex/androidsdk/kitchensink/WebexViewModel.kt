...
class WebexViewModel(val webex: Webex, val repository: WebexRepository) : BaseViewModel() {
	...
	private val permissionsHelper by lazy { PermissionsHelper(KitchenSinkApp.applicationContext()) }
	private val appContext: Context get() = KitchenSinkApp.applicationContext()
	...
	sealed class AnnotationEvent {
		...
		object READY : AnnotationEvent()
		object STOPPED : AnnotationEvent()
		object OVERLAY_PERMISSION_REQUIRED : AnnotationEvent()
	}

	// Track pending Virtual Background action for retry after permission grant
	private sealed class PendingVbAction {
		data class Apply(val background: VirtualBackground, val mode: Phone.VirtualBackgroundMode) : PendingVbAction()
		data class Remove(val background: VirtualBackground) : PendingVbAction()
		data class Add(val file: LocalFile) : PendingVbAction()
	}
	private var pendingVbAction: PendingVbAction? = null

	// Pending dial retry state (used when SDK returns PERMISSION_REQUIRED)
	private var pendingDialInput: String? = null
	private var pendingDialIsPhoneNumber: Boolean = false
	private var pendingDialOption: MediaOption? = null

	// Pending answer retry state
	private var pendingAnswer: Pair<Call, MediaOption>? = null
		...
		webex.phone.dialPhoneNumber(input, option, CompletionHandler { result ->
			...
			if (result.isSuccessful) {
				...
				result.error?.let { error ->
					if (error.errorCode == WebexError.ErrorCode.PERMISSION_REQUIRED.code) {
						val missing = (error.data as? List<String>) ?: emptyList()
						Log.d("KS-PERM", "PERMISSION_REQUIRED in dialPhoneNumber; missing=$missing")
						pendingDialInput = input
						pendingDialIsPhoneNumber = true
						pendingDialOption = option
						_callingLiveData.postValue(WebexRepository.CallLiveData(WebexRepository.CallEvent.DialFailed, null, null, "Missing permissions: $missing", missingPermissions = missing))
						return@CompletionHandler
					}
					Log.d("KS-PERM", "dialPhoneNumber error code=${error.errorCode} msg=${error.errorMessage}")
		...
		webex.phone.dial(input, option, CompletionHandler { result ->
			...
			if (result.isSuccessful) {
				...
				result.error?.let { error ->
					if (error.errorCode == WebexError.ErrorCode.PERMISSION_REQUIRED.code) {
						val missing = (error.data as? List<String>) ?: emptyList()
						Log.d("KS-PERM", "PERMISSION_REQUIRED in dial; missing=$missing")
						pendingDialInput = input
						pendingDialIsPhoneNumber = false
						pendingDialOption = option
						_callingLiveData.postValue(WebexRepository.CallLiveData(WebexRepository.CallEvent.DialFailed, null, null, "Missing permissions: $missing", missingPermissions = missing))
						return@CompletionHandler
					}
					Log.d("KS-PERM", "dial error code=${error.errorCode} msg=${error.errorMessage}")
					when(error.errorCode){
						WebexError.ErrorCode.UNEXPECTED_ERROR.code  -> {
							if(error.errorMessage == "NonExistentCallPull") {
								_callingLiveData.postValue(WebexRepository.CallLiveData(WebexRepository.CallEvent.NonExistentCallPull, null, null, result.error?.errorMessage))
						...
						WebexError.ErrorCode.PERMISSION_REQUIRED.code -> {
							val missing = (error.data as? List<String>) ?: emptyList()
							_callingLiveData.postValue(WebexRepository.CallLiveData(WebexRepository.CallEvent.DialFailed, null, null, "Missing permissions: $missing", missingPermissions = missing))
						}
						else -> {
	...
	fun retryPendingDialIfAny() {
		val input = pendingDialInput
		val option = pendingDialOption
		if (input != null && option != null) {
			if (pendingDialIsPhoneNumber) dialPhoneNumber(input, option) else dial(input, option)
		}
		pendingDialInput = null
		pendingDialOption = null
		pendingDialIsPhoneNumber = false
	}
		...
		call.answer(mediaOption, CompletionHandler { result ->
			if (result.isSuccessful) {
				...
				result.error?.let { error ->
					if (error.errorCode == WebexError.ErrorCode.PERMISSION_REQUIRED.code) {
						val missing = (error.data as? List<String>) ?: emptyList()
						Log.d("KS-PERM", "PERMISSION_REQUIRED in answer; missing=$missing")
						pendingAnswer = Pair(call, mediaOption)
						_callingLiveData.postValue(
							WebexRepository.CallLiveData(
								WebexRepository.CallEvent.AnswerPermissionsRequired,
								call = call,
								errorMessage = "Missing permissions: $missing",
								missingPermissions = missing
							)
						)
					} else {
						Log.d("KS-PERM", "answer error code=${error.errorCode} msg=${error.errorMessage}")
						_callingLiveData.postValue(
							WebexRepository.CallLiveData(
	...
	fun retryPendingAnswerIfAny() {
		pendingAnswer?.let { pair ->
			val (call, option) = pair
			pendingAnswer = null
			answer(call, option)
		}
	}
	...
	fun startShare(callId: String, shareConfig: ShareConfig?) {
		...
		call?.let { _ ->
			// Deprecated path for legacy flows; consent-based overload is used from the Fragment
			_startShareLiveData.postValue(false)
		}
	}
...
fun handoffScreenCaptureConsent(callId: String, data: Intent) { }
	...
	fun initalizeAnnotations(renderer: AnnotationRenderer) {
		getCall(currentCallId.orEmpty())?.getLiveAnnotationHandle()?.let {annotations->
					...
					annotationRenderer = renderer.apply {
						setAnnotationRendererCallback(object : AnnotationRenderer.AnnotationRendererCallback {
							override fun onAnnotationRenderingReady() {
								...
								_annotationEvent.postValue(AnnotationEvent.READY)
							}
							...
							override fun onAnnotationRenderingStopped() {
								...
								_annotationEvent.postValue(AnnotationEvent.STOPPED)
							}
						})
						startRendering()
					}
			...
			// If handle isn't available or overlay not permitted, notify UI to request overlay permission
			_annotationEvent.postValue(AnnotationEvent.OVERLAY_PERMISSION_REQUIRED)
		}
	}
	...
	fun switchAudioMode(mode: Call.AudioOutputMode, handler: CompletionHandler<Boolean>) {
		// Bluetooth permission check happens inside SDK switch; here we can optionally check to provide better UX
	...
	fun addVirtualBackground(imgFile: LocalFile) {
		repository.addVirtualBackground(imgFile, CompletionHandler {
			if (it.isSuccessful) {
				...
				it.error?.let { err ->
					if (err.errorCode == WebexError.ErrorCode.PERMISSION_REQUIRED.code) {
						val missing = (err.data as? List<String>) ?: emptyList()
						pendingVbAction = PendingVbAction.Add(imgFile)
						_callingLiveData.postValue(WebexRepository.CallLiveData(WebexRepository.CallEvent.DialFailed, null, null, "Missing permissions: $missing", missingPermissions = missing))
						return@CompletionHandler
					}
				}
	...
	fun addVirtualBackground(imgFile: LocalFile, handler: CompletionHandler<VirtualBackground>) {
		repository.addVirtualBackground(imgFile, CompletionHandler { result ->
			if (!result.isSuccessful) {
	...
	fun applyVirtualBackground(background: VirtualBackground, mode: Phone.VirtualBackgroundMode) {
		repository.applyVirtualBackground(background, mode, CompletionHandler { result ->
			if (result.isSuccessful && result.data == true) {
				...
				result.error?.let { err ->
					if (err.errorCode == WebexError.ErrorCode.PERMISSION_REQUIRED.code) {
						val missing = (err.data as? List<String>) ?: emptyList()
						pendingVbAction = PendingVbAction.Apply(background, mode)
						_callingLiveData.postValue(WebexRepository.CallLiveData(WebexRepository.CallEvent.DialFailed, null, null, "Missing permissions: $missing", missingPermissions = missing))
						return@CompletionHandler
					}
				}
				_virtualBgError.postValue(result.error?.errorMessage)
			}
		})
	}

	fun removeVirtualBackground(background: VirtualBackground) {
		repository.removeVirtualBackground(background, CompletionHandler { result ->
			if (result.isSuccessful && result.data == true) {
				...
				result.error?.let { err ->
					if (err.errorCode == WebexError.ErrorCode.PERMISSION_REQUIRED.code) {
						val missing = (err.data as? List<String>) ?: emptyList()
						pendingVbAction = PendingVbAction.Remove(background)
						_callingLiveData.postValue(WebexRepository.CallLiveData(WebexRepository.CallEvent.DialFailed, null, null, "Missing permissions: $missing", missingPermissions = missing))
						return@CompletionHandler
					}
				}
	...
	fun retryPendingVbIfAny() {
		when (val action = pendingVbAction) {
			is PendingVbAction.Apply -> applyVirtualBackground(action.background, action.mode)
			is PendingVbAction.Remove -> removeVirtualBackground(action.background)
			is PendingVbAction.Add -> addVirtualBackground(action.file)
			null -> {}
		}
		pendingVbAction = null
	}
