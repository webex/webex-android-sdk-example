 ...
 * This allows the app to receive incoming calls and messages even when not in the foreground.
 */
class KitchenSinkForegroundService : Service() {
    ...
    override fun onCreate() {
        ...
        if (isForegroundServiceSupported()) {
            startForeground(NOTIFICATION_ID, getServiceOngoingNotification(this))
        }
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        intent?.let {
            if (it.getBooleanExtra(STOP_REQUEST, false)) {
                stopSelf()
            }
        }
        return START_STICKY
    ...
    private fun getServiceOngoingNotification(context: Context): Notification {
        ...
        // Launch SplashActivity which will handle routing to the appropriate screen
    ...
    companion object {
        private const val TAG = "KitchenSinkForegroundService"
        private const val CHANNEL_ID = "ks_01"
        private const val NOTIFICATION_ID = 0x111111
