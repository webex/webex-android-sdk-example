...
/**
 * FCM service helper that implements IDynamicModule.IFCMHelper.
 * This class handles FCM messages when delegated from the base module's FCM service.
 */
class KitchenSinkFCMService : KoinComponent, IDynamicModule.IFCMHelper {
    ...
    // Context holder for methods that need it
    private var serviceContext: Context? = null
    
    private fun getContext(): Context = serviceContext ?: KitchenSinkApp.applicationContext()
    
    private fun getNotificationManager(): NotificationManager? {
        return getContext().getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager?
    }
    
    private fun getApplication(): KitchenSinkApp = KitchenSinkApp.get()
                                ...
                                override fun onDisconnected(event: CallObserver.CallDisconnectedEvent?) {
                                    ...
                                    when(event){
                                        is OtherConnected -> {
                                            ...
                                            Toast.makeText(getContext(), "The Call has been answered by other person in the hunt group", Toast.LENGTH_LONG).show()
                                        }
                                    }
    ...
    private fun sendCallNotification(callInfo: Call?, caller: String? = null) {
        ...
        val intent = CallActivity.getIncomingIntent(getContext())
    ...
    private fun sendNotificationForCall(callId:String, number: String, spaceTitle: String) {
        ...
        val acceptIntent = CallActivity.getCallAcceptIntent(getContext(), callId = callId)
        val rejectIntent = CallRejectService.getCallRejectIntent(getContext(), callId = callId)
        val fullScreenIntent = LockScreenActivity.getLockScreenIntent(getContext(), callId = callId)

        val intentFlag = PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
        val acceptPendingIntent = PendingIntent.getActivity(getContext(), Constants.Intent.ACCEPT_REQUEST_CODE, acceptIntent, intentFlag)
        val rejectPendingIntent = PendingIntent.getService(getContext(), Constants.Intent.REJECT_REQUEST_CODE, rejectIntent, intentFlag)
        val fullScreenPendingIntent = PendingIntent.getActivity(getContext(), Constants.Intent.FULLSCREEN_REQUEST_CODE, fullScreenIntent, intentFlag)
        val channelId = "Calls"
        val defaultSoundUri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION)
        val notificationBuilder = NotificationCompat.Builder(getContext(), channelId)
                .setSmallIcon(BaseR.drawable.app_notification_icon)
