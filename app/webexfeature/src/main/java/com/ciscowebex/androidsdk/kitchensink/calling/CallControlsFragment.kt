...
class CallControlsFragment : Fragment(), OnClickListener, CallObserverInterface, RemoteShareCallback {
    ...
    private lateinit var screenShareView: com.ciscowebex.androidsdk.phone.RemoteShareTextureView
    private lateinit var ibHoldCall: ImageButton
    private lateinit var localView: MediaRenderView
    private lateinit var remoteView: MediaRenderView
    private lateinit var viewAuxVideos: ViewGroup
    private lateinit var tvName: TextView
    private lateinit var callingHeader: TextView
    private lateinit var btnReturnToMainSession: View
    private lateinit var ivReceivingNoiseRemoval: ImageView
    private lateinit var annotationPolicy: TextView
    private lateinit var ibTransferCall: ImageButton
    private lateinit var ibMerge: ImageButton
    ...
    private lateinit var ivCancelCall: ImageView
    private lateinit var ibSwapCamera: ImageButton
    ...
    private lateinit var ivRemoteAudioState: ImageView
    private lateinit var videoCallLayout: ViewGroup
    private lateinit var viewAuxVideosContainer: ViewGroup
    private lateinit var optionButtonsContainer: ViewGroup
    private lateinit var controlsRow2: ViewGroup
    private lateinit var controlsRow3: ViewGroup
    private lateinit var controlsRow4: ViewGroup
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        rootView = inflater.inflate(R.layout.fragment_call_controls, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        screenShareView.setRemoteShareCallback(this)
    ...
    private fun initializeViews(view: View) {
        screenShareView = view.findViewById(R.id.screenShareView)
        ibHoldCall = view.findViewById(R.id.ib_hold_call)
        localView = view.findViewById(R.id.localView)
        remoteView = view.findViewById(R.id.remoteView)
        viewAuxVideos = view.findViewById(R.id.view_aux_videos)
        tvName = view.findViewById(R.id.tv_name)
        callingHeader = view.findViewById(R.id.calling_header)
        btnReturnToMainSession = view.findViewById(R.id.btn_return_to_main_session)
        ivReceivingNoiseRemoval = view.findViewById(R.id.iv_receiving_noise_removal)
        annotationPolicy = view.findViewById(R.id.annotation_policy)
        ibTransferCall = view.findViewById(R.id.ib_transfer_call)
        ibMerge = view.findViewById(R.id.ib_merge)
        ibAdd = view.findViewById(R.id.ib_add)
        ibVideo = view.findViewById(R.id.ib_video)
        ibSwitchToAudioVideoCall = view.findViewById(R.id.ib_switch_to_audio_video_call)
        ibMute = view.findViewById(R.id.ib_mute)
        mainContentLayout = view.findViewById(R.id.main_content_layout)
        incomingCallHeader = view.findViewById(R.id.incoming_call_header)
        ibParticipants = view.findViewById(R.id.ib_participants)
        ibAudioMode = view.findViewById(R.id.ib_audio_mode)
        ibDirecttransferCall = view.findViewById(R.id.ib_directtransfer_call)
        ivCancelCall = view.findViewById(R.id.iv_cancel_call)
        ibSwapCamera = view.findViewById(R.id.ib_swap_camera)
        ibScreenShare = view.findViewById(R.id.ib_screen_share)
        ibMoreOption = view.findViewById(R.id.ib_more_option)
        ivNetworkSignal = view.findViewById(R.id.iv_network_signal)
        controlGroup = view.findViewById(R.id.controlGroup)
        localViewLayout = view.findViewById(R.id.localViewLayout)
        remoteViewLayout = view.findViewById(R.id.remoteViewLayout)
        tvRemoteUserName = view.findViewById(R.id.tv_remote_user_name)
        ivRemoteAudioState = view.findViewById(R.id.iv_remote_audio_state)
        videoCallLayout = view.findViewById(R.id.video_call_layout)
        viewAuxVideosContainer = view.findViewById(R.id.view_aux_videos_container)
        optionButtonsContainer = view.findViewById(R.id.option_buttons_container)
        controlsRow2 = view.findViewById(R.id.controlsRow2)
        controlsRow3 = view.findViewById(R.id.controlsRow3)
        controlsRow4 = view.findViewById(R.id.controlsRow4)
    }
    ...
    private fun checkIsOnHold() {
        ...
        ibHoldCall.isSelected = isOnHold ?: false
    }
    ...
    private fun getMediaOption(isModerator: Boolean = false, pin: String = "", captcha: String = "", captchaId: String = "", companionMode: CompanionMode = CompanionMode.None): MediaOption {
        ...
        if (webexViewModel.callCapability == WebexRepository.CallCap.Audio_Only) {
            ...
            mediaOption = MediaOption.audioVideoSharing(Pair(localView, remoteView), screenShareView)
           // MediaOption.audioVideo(Pair(localView, remoteView))
        }
    ...
    override fun onJoinedSessionUpdated(breakoutSession: BreakoutSession) {
        lifecycleScope.launch(Dispatchers.Main) {
            tvName.text = breakoutSession.getName()
        }
    }
    ...
    override fun onReturnedToMainSession() {
        ...
        lifecycleScope.launch(Dispatchers.Main) {
            callingHeader.text = getString(R.string.onCall)
            tvName.text = callInfo?.getTitle()
            btnReturnToMainSession.visibility = View.INVISIBLE
        }
    }
    ...
    override fun onSessionJoined(breakoutSession: BreakoutSession) {
        ...
        lifecycleScope.launch(Dispatchers.Main) {
            callingHeader.text = getString(R.string.breakout_session)
            tvName.text = breakoutSession.getName()
            btnReturnToMainSession.visibility = View.VISIBLE
        }
    }
    ...
    override fun onReceivingNoiseInfoChanged(info: ReceivingNoiseInfo) {
        ...
        ivReceivingNoiseRemoval.visibility = View.VISIBLE
        if (info.isNoiseDetected() && !info.isNoiseRemovalEnabled()) {
            ivReceivingNoiseRemoval.setImageResource(R.drawable.ic_noise_detected_filled)
        } else if (!info.isNoiseRemovalEnabled()) {
            ivReceivingNoiseRemoval.setImageResource(R.drawable.ic_noise_none_filled)
        } else if (info.isNoiseRemovalEnabled()) {
            ivReceivingNoiseRemoval.setImageResource(R.drawable.ic_noise_detected_canceled_filled)
        }
    }

    override fun onClosedCaptionsArrived(captions: CaptionItem) {
        CoroutineScope(Dispatchers.Main).launch {
            captionsController.showCaptionView(rootView, captions)
            if(captions.isFinal) {
    ...
    private fun observerCallLiveData() {
            ...
            status?.let {
                if (it) {
                    ...
                    if(BuildConfig.FLAVOR != "wxc") {
                        annotationPolicy.visibility = VISIBLE
                        annotationPolicy.text = webexViewModel.getCurrentLiveAnnotationPolicy().toString()
                    }
        ...
        webexViewModel.stopShareLiveData.observe(viewLifecycleOwner, Observer { status ->
            ...
            annotationPolicy.visibility = INVISIBLE
        })
                ...
                _call?.let {
                    when {
                        (it.isCUCMCall() || it.isWebexCallingOrWebexForBroadworks()) && webexViewModel.isAddedCall -> {
                            ibTransferCall.visibility = View.VISIBLE
                            ibMerge.visibility = View.VISIBLE
                            ibAdd.visibility = View.INVISIBLE
                            ibVideo.visibility = View.INVISIBLE
                        }
                        it.isWebexCallingOrWebexForBroadworks() && webexViewModel.isVideoEnabled() -> {
                            ibSwitchToAudioVideoCall.visibility = View.VISIBLE
                        }
                        ...
                        !it.isCUCMCall() && !it.isWebexCallingOrWebexForBroadworks() -> {
                            ...
                            ibTransferCall.visibility = View.INVISIBLE
                        }
                    }
                }
            }
    ...
    private fun setUpViews(bundle: Bundle?) {
        ...
        if (callingActivity == 1) {
            ...
            incomingCallHeader.visibility = View.VISIBLE
            ...
            if(acceptedCall){
                _call?.let {
                    if(it.getStatus() != Call.CallStatus.CONNECTED) { // For resumed fragments
                        ...
                        incomingCallHeader.visibility = View.GONE
        ...
        ibParticipants.setOnClickListener(this)
        ibAudioMode.setOnClickListener(this)
        ibAdd.setOnClickListener(this)
        ibTransferCall.setOnClickListener(this)
        ibDirecttransferCall.setOnClickListener(this)
        ibDirecttransferCall.visibility = View.GONE

        ibHoldCall.setOnClickListener(this)
        ivCancelCall.setOnClickListener(this)
        ibVideo.setOnClickListener(this)
        ibSwapCamera.setOnClickListener(this)
        ibMerge.setOnClickListener(this)
        ibScreenShare.setOnClickListener(this)
        mainContentLayout.setOnClickListener(this)
        ibMoreOption.setOnClickListener(this)
        btnReturnToMainSession.setOnClickListener(this)
        ibSwitchToAudioVideoCall.setOnClickListener(this)
        ivReceivingNoiseRemoval.setOnClickListener(this)

        initAddedCallControls()
        ivNetworkSignal.setOnClickListener(this)
        ivNetworkSignal.visibility = View.GONE
        btnReturnToMainSession.visibility = INVISIBLE

        passwordDialog = Dialog(requireContext())

        annotationPolicy.setOnClickListener(this)
    }
    ...
    override fun onClick(v: View?) {
        webexViewModel.currentCallId?.let { callId ->
            when (v) {
                ...
                btnReturnToMainSession -> {
                    webexViewModel.returnToMainSession()
                }
                ivReceivingNoiseRemoval -> {
    ...
    private fun showPolicySelectionListDialog() {
        ...
        builder.setSingleChoiceItems(policyList(), 0) { dialog, which ->
            ...
            annotationPolicy.text = policyList()[which]
            dialog.dismiss()
        }
        builder.show()
    }
    ...
    private fun mainContentLayoutClickListener() {
        ...
        if (controlGroup.visibility == View.VISIBLE) {
    ...
    private fun screenShareButtonVisibilityState() {
        webexViewModel.currentCallId?.let {
            ...
            if (canShare) {
                ibScreenShare.visibility = VISIBLE
            } else {
                ibScreenShare.visibility = INVISIBLE
            }
        ...
        webexViewModel.currentCallId?.let {callId ->
            ...
            call?.let {
                if (it.isWebexCallingOrWebexForBroadworks() && !it.isGroupCall()) {
                    ibDirecttransferCall.visibility = VISIBLE
                }
            }
        }
    }
    ...
    private fun updateScreenShareButtonState(state: ShareButtonState) {
        when (state) {
            ShareButtonState.OFF -> {
                ibScreenShare.isEnabled = true
                ibScreenShare.alpha = 1.0f
                ibScreenShare.background = ContextCompat.getDrawable(requireActivity(), R.drawable.screen_sharing_default)
            }
            ShareButtonState.ON -> {
                ibScreenShare.isEnabled = true
                ibScreenShare.alpha = 1.0f
                ibScreenShare.background = ContextCompat.getDrawable(requireActivity(), R.drawable.screen_sharing_active)
            }
            ShareButtonState.DISABLED -> {
                ibScreenShare.isEnabled = false
                ibScreenShare.alpha = 0.5f
            }
        }
    }
    ...
    private fun videoViewTextColorState(hidden: Boolean) {
        ...
        if (hide) {
            callingHeader.setTextColor(ContextCompat.getColor(requireContext(), R.color.black))
            tvName.setTextColor(ContextCompat.getColor(requireContext(), R.color.black))
        } else {
            val status = isMainStageRemoteVideoUnMuted()
            if (status) {
                callingHeader.setTextColor(ContextCompat.getColor(requireContext(), R.color.white))
    ...
    private fun localVideoViewState(toHide: Boolean) {
        if (toHide) {
            localViewLayout.visibility = View.GONE
            ibSwapCamera.visibility = View.GONE
        } else {
            localViewLayout.visibility = View.VISIBLE
            ibSwapCamera.visibility = View.VISIBLE
    ...
    private fun screenShareViewRemoteState(toHide: Boolean, needResize: Boolean = true) {
        ...
        if (toHide) {
            screenShareView.visibility = View.GONE
    ...
    private fun resizeRemoteVideoView() {
        ...
        if (webexViewModel.isRemoteScreenShareON) {
            ...
            remoteViewLayout.background = ContextCompat.getDrawable(requireActivity(), R.drawable.surfaceview_border)
            ...
            remoteViewLayout.layoutParams = params
            remoteViewLayout.background = ContextCompat.getDrawable(requireActivity(), R.drawable.surfaceview_transparent_border)
    ...
    private fun videoViewState(toHide: Boolean) {
        ...
        if (toHide) {
            remoteViewLayout.visibility = View.GONE
        } else {
            val status = isMainStageRemoteVideoUnMuted()
            if (status) {
                remoteViewLayout.visibility = View.VISIBLE
            }
        }
    ...
    private fun videoButtonState(videoViewHidden: Boolean) {
        if (videoViewHidden) {
            ibVideo.background = ContextCompat.getDrawable(requireActivity(),
                R.drawable.turn_off_video_active
            )
        } else {
            ibVideo.background = ContextCompat.getDrawable(requireActivity(), R.drawable.turn_on_video_default)
        }
    }
    ...
    private fun onCallConnected(callId: String, isCucmCall: Boolean, isWebexCallingOrWebexForBroadworks: Boolean) {
        ...
        mHandler.post {
            ...
            ivNetworkSignal.visibility = View.VISIBLE
            ...
            if (!webexViewModel.multistreamNewApproach) {
                tvRemoteUserName.visibility = View.GONE
                ivRemoteAudioState.visibility = View.GONE
    ...
    private fun updateAudioModeButton() {
        webexViewModel.getCurrentAudioOutputMode()?.let { outputMode ->
            when (outputMode) {
                Call.AudioOutputMode.PHONE -> ibAudioMode.setImageResource(R.drawable.ic_earpiece)
                Call.AudioOutputMode.SPEAKER -> ibAudioMode.setImageResource(R.drawable.ic_speaker)
                Call.AudioOutputMode.BLUETOOTH_HEADSET -> ibAudioMode.setImageResource(R.drawable.ic_bluetooth)
                Call.AudioOutputMode.HEADSET -> ibAudioMode.setImageResource(R.drawable.ic_headset)
            }
        }
    }
        ...
        mHandler.post {
            ...
            if (remoteSharing) {
                ...
                if (view == null || (remoteStartedSharing != null)) {
                    webexViewModel.setSharingRenderView(callId, screenShareView)
                }
            }
            else {
                onVideoStreamingChanged(callId)
    ...
    private fun setRemoteVideoInformation(name: String, audioMuted: Boolean) {
        tvRemoteUserName.text = name

        if (audioMuted) {
            ivRemoteAudioState.setImageResource(R.drawable.ic_microphone_muted_bold)
        } else {
            ivRemoteAudioState.setImageResource(R.drawable.ic_microphone_36)
        }
    }
                    ...
                    if (status) {
                        ...
                        if (webexViewModel.streamMode != Phone.VideoStreamMode.COMPOSITED) {
                            if (!webexViewModel.multistreamNewApproach) {
                                ...
                                ivRemoteAudioState.visibility = View.VISIBLE
                                tvRemoteUserName.visibility = View.VISIBLE
                            }
                        } else {
                            ivRemoteAudioState.visibility = View.GONE
                            tvRemoteUserName.visibility = View.GONE
        ...
        when (mode) {
            AudioMode.SPEAKER -> {
                webexViewModel.switchAudioMode(Call.AudioOutputMode.SPEAKER) {
                    if (it.data == true)
                        ibAudioMode.setImageResource(R.drawable.ic_speaker)
                    else {
            ...
            AudioMode.BLUETOOTH -> {
                webexViewModel.switchAudioMode(Call.AudioOutputMode.BLUETOOTH_HEADSET) {
                    if (it.data == true)
                        ibAudioMode.setImageResource(R.drawable.ic_bluetooth)
                    else {
                        val perms = PermissionsHelper.permissionForBluetooth()
                        Toast.makeText(requireContext(), "Bluetooth permission required", Toast.LENGTH_LONG).show()
                        permissionRequestForBluetooth.launch(perms)
            ...
            AudioMode.EARPIECE -> {
                webexViewModel.switchAudioMode(Call.AudioOutputMode.PHONE) {
                    if (it.data == true)
                        ibAudioMode.setImageResource(R.drawable.ic_earpiece)
                    else {
                        val perms = PermissionsHelper.permissionForBluetooth()
                        Toast.makeText(requireContext(), "Bluetooth permission required", Toast.LENGTH_LONG).show()
                        permissionRequestForBluetooth.launch(perms)
            ...
            AudioMode.WIRED_HEADSET -> {
                webexViewModel.switchAudioMode(Call.AudioOutputMode.HEADSET) {
                    if (it.data == true)
                        ibAudioMode.setImageResource(R.drawable.ic_headset)
                    else {
                        val perms = PermissionsHelper.permissionForBluetooth()
                        Toast.makeText(requireContext(), "Bluetooth permission required", Toast.LENGTH_LONG).show()
                        permissionRequestForBluetooth.launch(perms)
        ...
        mHandler.post {
            if (call?.getCallId().orEmpty() == webexViewModel.currentCallId) {
                ...
                call?.let {
                    ...
                    schedules?.let {
                        callingHeader.text = getString(R.string.meeting)
                    }
                }
            }
            if (callingActivity == 1) {
            ...
            Log.d(TAG,"CallControlsFragment callingHeader text: ${callingHeader.text}")
        }
    }
    ...
    private fun initAddedCallControls() {
        ...
        ibMerge.visibility = View.INVISIBLE
    }
    ...
    private fun swapVideoClickListener(call: Call?) {
        ...
        if (webexViewModel.isVideoViewsSwapped) {
            webexViewModel.setVideoRenderViews(webexViewModel.currentCallId.orEmpty(), remoteView, localView)
            webexViewModel.isVideoViewsSwapped = false
        } else {
            webexViewModel.setVideoRenderViews(webexViewModel.currentCallId.orEmpty(), localView, remoteView)
    ...
    private fun updateNetworkStatusChange(mediaQualityInfo: Call.MediaQualityInfo) {
        when (mediaQualityInfo) {
            Call.MediaQualityInfo.NetworkLost -> {
                ivNetworkSignal.setImageResource(R.drawable.ic_no_network)
                currentNetworkStatus = NetworkStatus.NoNetwork
            }
            Call.MediaQualityInfo.Good -> {
                ivNetworkSignal.setImageResource(R.drawable.ic_good_network)
                currentNetworkStatus = NetworkStatus.Good
            }
            Call.MediaQualityInfo.PoorUplink -> {
                ivNetworkSignal.setImageResource(R.drawable.ic_poor_network)
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        ...
        localViewLayout.requestLayout()
    }

    fun pipVisibility(currentView: Int, inPipMode: Boolean) {
            isInPipMode = inPipMode
        if(currentView == View.GONE){
            videoCallLayout.layoutParams.height=400
            if(screenShareView.isVisible){
                remoteViewLayout.visibility=currentView
            }
        }else{
            videoCallLayout.layoutParams.height = resources.getDimension(R.dimen.video_view_height).toInt()
            remoteViewLayout.visibility=currentView
        }
        localViewLayout.visibility = currentView
        viewAuxVideosContainer.visibility = currentView
        ivNetworkSignal.visibility = currentView
        tvRemoteUserName.visibility = currentView
        ivRemoteAudioState.visibility = currentView
        callingHeader.visibility = currentView
        tvName.visibility = currentView
        optionButtonsContainer.visibility = currentView
        ibMute.visibility = currentView
        ibHoldCall.visibility = currentView
        ibAudioMode.visibility = currentView
        controlsRow2.visibility = currentView
        ibVideo.visibility = currentView
        ibParticipants.visibility = currentView
        controlsRow3.visibility = currentView
        ibScreenShare.visibility = currentView
        ibSwapCamera.visibility = currentView
        ibMoreOption.visibility = currentView
        ivCancelCall.visibility = currentView
        controlsRow4.visibility = currentView
        ibSwitchToAudioVideoCall.visibility = currentView
        ...
        btnReturnToMainSession.visibility = returnToMainSessionVisibility
    }

    fun aspectRatio(): Rational {
        var width = videoCallLayout.width.toInt()
        var height = videoCallLayout.height.toInt()
