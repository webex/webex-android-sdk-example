...
class SpacesFragment : Fragment() {
    ...
    private lateinit var addSpacesFAB: FloatingActionButton
    private lateinit var spacesRecyclerView: androidx.recyclerview.widget.RecyclerView
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        menu.getItem(0).isChecked = spacesRecyclerView.adapter is SpaceReadStatusClientAdapter
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId) {
            ...
            R.id.spaceReadStatus -> {
                ...
                if (item.isChecked) {
                    spacesRecyclerView.adapter = spacesReadClientAdapter
                } else {
                    spacesRecyclerView.adapter = spacesClientAdapter
                }
            }
            R.id.spaceFilter -> {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        addSpacesFAB = view.findViewById(R.id.addSpacesFAB)
        spacesRecyclerView = view.findViewById(R.id.spacesRecyclerView)
    ...
    private fun checkForInitialSpacesSync() {
        if (!spacesViewModel.isSpacesSyncCompleted()) {
            view?.let { Snackbar.make(it, getString(R.string.syncing_spaces), Snackbar.LENGTH_SHORT).show() }
        }
    }
        ...
        spacesViewModel.spacesSyncCompletedLiveData.observe(viewLifecycleOwner) { isSyncing ->
            view?.let { rootView ->
                if (isSyncing) {
                    ...
                    Snackbar.make(rootView, getString(R.string.not_syncing_spaces), Snackbar.LENGTH_SHORT).show()
                }
            }
        }
